...
:::demo 将 `options` 属性分配给选项数组会呈现 Cascader。 `props.expandTrigger` 属性定义如何扩展子选项。
...
为`el-cascader`设置`clearable`属性，选中并悬停时会出现清晰的图标
...
:::demo 使用多重选择时，默认显示所有选定的标签。您可以设置 `collapse-tags = true` 折叠选定的标签。您可以设置 `max-collapse-tags` 显示最大标签数，默认 1。您可以使用 `collapse-tags-tooltip` 属性在鼠标悬停折叠文本时检查它们。
...
| 折叠标签工具提示                   | 当鼠标悬停在折叠标签的文本上时是否显示所有选定的标签。要使用此功能，`collapse-tags` 必须为 true      | ^[布尔值]                                                                                                                                                                   | 假       |
...
| 传送                               | Cascader 弹出窗口是否被传送                                                                          | ^[布尔值]                                                                                                                                                                   | 真实     |
| 效果 ^(2.10.5)                     | 工具提示主题，内置主题：`dark` / `light`                                                             | ^[枚举]`'dark' \| 'light'` / ^[字符串]                                                                                                                                      | 光       |
...
| 后备放置 ^(2.8.1)                  | 工具提示的可能位置列表 [popper.js](https://popper.js.org/docs/v2/modifiers/flip/#fallbackplacements) | ^[数组]`Placement[]`                                                                                                                                                        | —        |
| 放置 ^(2.8.1)                      | 下拉菜单的位置                                                                                       | ^[枚举]`'top' \| 'top-start' \| 'top-end' \| 'bottom' \| 'bottom-start' \| 'bottom-end' \| 'left' \| 'left-start' \| 'left-end' \| 'right' \| 'right-start' \| 'right-end'` | 底部开始 |
| popper-append-to-body ^（已弃用）  | 是否将弹出菜单附加到正文。如果 popper 的定位错误，可以尝试将此属性设置为 false                       | ^[布尔值]                                                                                                                                                                   | 真实     |
...
| 属性                       | 描述                                                                                 | 类型                                                                | 默认   |
| -------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------- | ------ | --- |
| 展开触发                   | 扩展选项触发方式                                                                     | ^[枚举]`'click' \| 'hover'`                                         | 点击   |
| 多个                       | 是否启用多选                                                                         | ^[布尔值]                                                           | 假     |
