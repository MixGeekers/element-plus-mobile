...
以下示例统一使用移动端预览壳呈现，便于直接检查当前默认的触控布局、按钮排版与校验提示。
...
`Form` 现在默认使用更适合移动端的堆叠式布局，包括标签置顶、错误信息回流到文档流，以及统一的触控尺寸。

:::demo 这个示例直接展示当前默认的表单交互。展开 `Select / SelectV2` 可以继续检查底部弹层和多选确认流程。
...
| 规则     | 表单校验规则见 [下表](#表单项规则)，更高级的用法见 [async-validator](https://github.com/yiminghe/async-validator)。                    | ^[对象]`Arrayable<FormItemRule>`                    | —    |
...
| 名称    | 描述               | 类型                        | 默认 |
| ------- | ------------------ | --------------------------- | ---- |
| trigger | 验证器的触发时机。 | ^[枚举]`'blur' \| 'change'` | —    |
