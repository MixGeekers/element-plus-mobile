...
The calendar and time selectors are presented inside the same bottom sheet flow.

## Date and time

:::demo You can select date and time in one picker at the same time by setting `type` to `datetime`. On mobile, both steps stay inside the same bottom-sheet flow.
...
```ts
// DateTimePicker reuses DatePicker and TimePicker value types.
```
