...
The picker opens as a bottom sheet, and range selection keeps both endpoints in one synchronized flow.
...
:::demo On mobile, range selection keeps the two endpoints in one synchronized flow and can still be accelerated with shortcuts.
...
:::demo Month range selection follows the same bottom-sheet interaction and works well for reporting windows or billing periods.
...
:::demo Year range selection uses the same mobile sheet pattern for longer planning horizons.
