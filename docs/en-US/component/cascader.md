...
Child option items expand by tap.

:::demo Assigning the `options` attribute to an array of options renders a Cascader. Mobile mode always expands child options on tap and keeps the active menu column in view.
...
Set `clearable` attribute for `el-cascader` and a clear icon will appear when a value is selected.
...
:::demo When using multiple selection, all selected tags will display by default. You can set `collapse-tags = true` to fold selected tags. You can set `max-collapse-tags` to show the maximum number of visible tags, default 1.
...
| collapse-tags-tooltip                      | whether to show all selected tags from the collapsed summary. To use this, `collapse-tags` must be true                                                                          | ^[boolean]                                                    | false       |
...
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | -------- |
| expandTrigger              | trigger mode of expanding options. `el-cascader` always uses `click`; this option remains relevant when using `CascaderPanel` directly. | ^[enum]`'click' \| 'hover'`                                             | click    |
...
| hoverThreshold             | hover threshold of expanding options when `CascaderPanel` is configured for hover expansion                                             | ^[number]                                                               | 500      |
