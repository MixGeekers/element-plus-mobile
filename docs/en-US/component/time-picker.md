...
The time list opens as a bottom sheet instead of desktop-style arrow controls.

## Arbitrary time picker

Can pick an arbitrary time.

:::demo By default, you can scroll the time list to pick a value. The mobile sheet keeps the interaction in one vertical flow.
...
:::demo We can pick a time range by adding an `is-range` attribute. Range mode keeps both endpoints grouped in the same mobile sheet.
...
```ts
// TimePicker emits Date values by default and supports string/number formatting through value-format.
```
